...
-- 2. Conversion between scope-safe and raw term representation (the latter is generated via BNFC), see 'toLambdaPi' and 'fromLambdaPi'.
-- 3. Computation of weak head normal form (WHNF), see 'whnf'.
-- 4. Entry point, gluing everything together. See 'defaultMain'.
-- 5. 'ZipMatchK' instances (enabling general \(\alpha\)-equivalence), written out
--    for each signature. See "Language.LambdaPi.Impl.FreeFoilTH" for the same
--    instances generated by 'Data.ZipMatchK.TH.deriveZipMatchK2' instead.
--
...
-- | Matching two \(\lambda\Pi\)-nodes, written out by hand.
--
-- Matching the collection of zipping functions against @f ':^:' g ':^:' 'M0'@
-- is what tells GHC that the two type parameters are the scoped terms and the
-- terms, so that the constructors below typecheck: @f@ zips the scoped terms
-- and @g@ zips the terms.
--
-- This is what 'Data.ZipMatchK.TH.deriveZipMatchK' generates, and what
-- "Language.LambdaPi.Impl.FreeFoilTH" uses it for.
instance ZipMatchK LambdaPiF where
  zipMatchWithK (f :^: g :^: M0) l r = case (l, r) of
    (AppF l1 l2, AppF r1 r2) -> AppF <$> g l1 r1 <*> g l2 r2
    (LamF l1, LamF r1)       -> LamF <$> f l1 r1
    (PiF l1 l2, PiF r1 r2)   -> PiF <$> g l1 r1 <*> f l2 r2
    (UniverseF, UniverseF)   -> Just UniverseF
    _                        -> Nothing
...
-- | Matching two nodes of the pair signature, written out by hand.
instance ZipMatchK PairF where
  zipMatchWithK (_f :^: g :^: M0) l r = case (l, r) of
    (PairF l1 l2, PairF r1 r2)       -> PairF <$> g l1 r1 <*> g l2 r2
    (FirstF l1, FirstF r1)           -> FirstF <$> g l1 r1
    (SecondF l1, SecondF r1)         -> SecondF <$> g l1 r1
    (ProductF l1 l2, ProductF r1 r2) -> ProductF <$> g l1 r1 <*> g l2 r2
    _                                -> Nothing
