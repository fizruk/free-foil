...
- `alphaEquiv` threads the verdict's renamings down the recursion instead of applying them. The eager version materialised a renamed copy of the whole remaining body at every mismatched binder, which is quadratic on a chain of them: two α-equivalent 2000-binder chains cost 353 ms and 2.1 GB of allocation for one comparison, three orders of magnitude behind `alphaEquivRefreshed`, while the haddocks claimed the opposite. The comparison now runs in two modes — the old stateless fast path while matching binders coincide, and a worker carrying one environment per side (raw name to the level of the binder pair that bound it, consulted at variables only) from the first mismatch until the environments empty out. Levels are what make threading sound: a level is never a name, so a renamed bound name cannot be conflated with a free name sharing the target spelling, and there is nothing for an inner binder to capture. The rename path is now linear (211 µs at depth 2000) and the fast path is unchanged; `bench/alphaequiv` keeps the adversarial chain measured, and a spec pins the conflation cases, shadowing, and agreement with `alphaEquivRefreshed` on random pairs.

- `substitute` short-circuits on the empty substitution, returning the term by the coercion `sink` performs. An empty substitution maps every name to itself, since `addRename` deletes identity renames; and substitutions go empty often — opening a scoped term with its own binder's name is an identity rename, and under a deterministic allocation policy a refreshed binder usually keeps its name. Under rzk on the sHoTT corpus 74% of `substitute`'s node visits ran with an empty substitution, and the short-circuit speeds the whole check up by about 4%. Binders that shadow the ambient scope are left as they stand, as on the no-clash path; a caller that wants them refreshed asks `substituteRefreshed`, which is unchanged. `nullSubst` is exported for clients that want the same test.
...
- `mkFreeFoil` now generates a concrete `Foil.CoSinkable` instance for each binding type: one clause per constructor, `coSinkabilityProof` threading the renaming and `withPattern` threading the scope left to right across binder and nested-binding fields, payloads passing through untouched. Previously the client declared an empty instance, whose GenericK-default `withPattern` routes every binder operation (`withRefreshedPattern`, `extendScopePattern`, and friends) through a generic representation traversal; on the free-foil-NBE normaliser that traversal cost about 40% of normalisation time on the lambda-n-ways random corpus ([#82](https://github.com/fizruk/free-foil/issues/82)). A binding constructor whose payload would be scope-indexed is refused at generation time with a pointer to `transportPayload`, matching the GenericK-side refusal for derived patterns.

Performance:

- The binder and substitution operations (`lookupSubst`, `addSubst`, `addRename`, `extendScope`, `extendScopePattern`, `withFreshBinder`, `withFresh`, `withRefreshed`, `withRefreshedPattern`, `unsafeAssertFresh`) are now `INLINABLE`, so a downstream `SPECIALIZE` reaches through them instead of stopping at the module boundary with dictionary-passing calls to the `CoSinkable`-constrained ones. Measured on the free-foil-NBE normaliser after its own specialisation: about −4% allocation, time within noise ([#83](https://github.com/fizruk/free-foil/issues/83)). No behaviour change.

- The name in the free foil's `Var` is unpacked. A lazy field held a pointer to a boxed `Int`; GHC preallocates shared boxes for the values −16…16, so a variable with a small index pointed at one of those and cost nothing, while a larger index allocated a fresh two-word box per occurrence. The unpacking removes that box. Neutral on the benchmarked corpora, whose indices are all small.

Changed:

- **Clients of `mkFreeFoil` must delete their own `Foil.CoSinkable` instances** for the generated binding types (whether empty or hand-written), since the generated instance would otherwise be a duplicate. The `soas` and `mltt` packages in this repository are updated accordingly. `SinkableK`, `HasNameBinders` and `UnifiablePattern` remain the client's to declare.

- **`withPattern`'s continuation now also receives the scope extended by the pattern**, and so do the continuations of `withRefreshedPattern`, `withFreshPattern` and `withRefreshedPattern'` (an extra argument after the pattern). Every implementation of `withPattern` threads the extended scope binder by binder and then discarded it, so every caller of the refreshers recomputed it with `extendScopePattern`, a second fold over the same pattern; the traversal now hands the scope out where it is known — the `NameBinder` instance pays the one `extendScope` its binder stands for, a case that binds nothing passes the ambient scope through (well-typed with no coercion, since its GADT return type pins the scopes equal), and the refresher accumulators stay the plain newtypes they were ([#84](https://github.com/fizruk/free-foil/issues/84)). Migration is mechanical on both levels: a caller adds the argument to its continuation and deletes the `extendScopePattern` call; a hand-written `CoSinkable` instance passes the scope its traversal already computes, and instances generated by `mkFreeFoil` are updated automatically. Every call site and instance in this repository needed exactly that. On the free-foil-NBE normaliser the change is about −11% allocation and −7% time on the lambda-n-ways random corpus.
