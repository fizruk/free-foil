...
substituteHelper substTerm substName = \case
  ...
  FoilPairTerm term1 term2 -> FoilPairTerm (substituteHelper substTerm substName term1) (substituteHelper substTerm substName term2)
  FoilLit n -> FoilLit n
  FoilApp term1 term2 -> FoilApp (substituteHelper substTerm substName term1) (substituteHelper substTerm substName term2)
  FoilLam (FoilPatternLit pat) (FoilScopedTerm term) -> 
    FoilLam (FoilPatternLit pat) (FoilScopedTerm (substituteHelper substTerm (UnsafeName (ppName substName)) term))
  FoilLam (FoilPatternVar pat) (FoilScopedTerm term)
    | ppName (nameOf pat) == ppName substName -> 
      FoilLam (FoilPatternVar newPat) (FoilScopedTerm (substituteHelper substTerm (UnsafeName "") term))
    | otherwise -> 
      FoilLam (FoilPatternVar newPat) (FoilScopedTerm (substituteHelper substTerm (UnsafeName (ppName substName)) term))
      where
        newPat = UnsafeNameBinder (UnsafeName (ppName (nameOf pat)))

simpleTerm :: Term
simpleTerm = Lam (PatternVar "s") (ScopedTerm (Lam (PatternVar "z") (ScopedTerm (App (Var "s") (App (Var "s") (Var "z"))))))
...
foilName :: Name n
foilName = UnsafeName "s" :: Name n

foilTerm :: FoilTerm n
foilTerm = FoilVar (UnsafeName "zz" :: Name n)
